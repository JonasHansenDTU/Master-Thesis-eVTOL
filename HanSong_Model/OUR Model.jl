...
function build_model(excel_file::String, parameter_file::String; show_progress::Bool = true, display_interval_sec::Int = 5)
    ...
    @constraint(model, [m in M_mid, n in N],
        sum(x[i,j,m,n] for i in V, j in endVP[bv[n]]) >=
    ...
    @constraint(model, [n in N],
        sum(x[i,j,maximum(M),n] for i in V, j in endVP[bv[n]]) >=
        sum(x[i,j,maximum(M),n] for i in V, j in V)
    )

    # (6.4) eVTOL 
    @constraint(model, [m in 1:maximum(M), n in N], 
        sum(x[i,j,m,n] for i in V, j in V) <= y[n] 
    )

    # (6.5) 
    @constraint(model, [i in V, m in M_no0, n in N], 
        x[i,i,m,n] <= 0
    )
    ...
    # (6.16) Upper limit on how many k's can be 1 for each passenger group,
    @constraint(model, [a in A],
        sum(k[a,i,j,m,n] for m in M, i in V, j in V, n in N) <= 1 + z[a]
    )

    # (6.17a) Layover service linkage 
    ...
    @constraint(model, [m in M_no0, n in N], 
        bp[m,n] >= u[m,n] - bmid + sum(e[(i,j)] * x[i,j,m,n] for i in V, j in V)
    )
    ...
    @constraint(model, [j in V, n in N, t in T],
        is_p[j,n,t] <= sum(is_o[i,k,m,n,t2] for i in V, k in V, m in M, t2 in t:maximum(T))
    )
...
excel_file = joinpath("inputData/Experiments/inputDataEx2_1_2.xlsx")
