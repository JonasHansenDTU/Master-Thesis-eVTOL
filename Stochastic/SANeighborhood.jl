function DestructSA(planes::allPlaneSolution, maxTurnaround::Int64, init_obj::Float64, data, rt, n_trials::Int=1)
    ...
    # Restrict to the active fleet if an active set is present (second stage);
    # otherwise use the full fleet (deterministic model).
    N = hasproperty(data, :active) ? collect(data.active) : data.N
    isempty(N) && return init_obj, planes
...
function ConstructSA(planes::allPlaneSolution, maxTurnaround::Int64, init_obj::Float64, data, rt)
    ...
    # Restrict to the active fleet if present (second stage); else full fleet.
    N = hasproperty(data, :active) ? collect(data.active) : data.N
    isempty(N) && return init_obj, planes
    V = data.V
    best_obj = init_obj
